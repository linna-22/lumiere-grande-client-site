...
export default function SuitesPage() {
  ...
  // useRoomsData already contains ONLY available rooms.
  const filteredRooms = useMemo(() => {
    return rooms.filter((room) => {
      ...
      const matchesPrice =
        room.pricePerNight >= effectiveRange[0] &&
        room.pricePerNight <= effectiveRange[1];
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredRooms.length / PER_PAGE)
  );
            ...
            {pagedRooms.length > 0 ? (
              ...
              <div className="mt-16 flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-slate-200 py-20 text-center">
                ...
                <p className="max-w-sm text-sm text-slate-500">
                  Try adjusting the room type or price range to see
                  more of our available accommodations.
                </p>
              </div>
            )}
          </>

        ...
        <div className="lg:col-span-2">
          ...
          <div className="mt-5 flex flex-wrap gap-x-8 gap-y-4 border-y border-slate-100 py-6 text-sm text-slate-600">
            <span className="flex items-center gap-2">
              <Maximize className="h-5 w-5 text-amber-600" /> {room.size} m²
            </span>
            <span className="flex items-center gap-2">
              <Users className="h-5 w-5 text-amber-600" /> Up to {room.capacity} Guests
            </span>
            <span className="flex items-center gap-2">
              <BedDouble className="h-5 w-5 text-amber-600" /> {room.beds}
            </span>
            <span className="flex items-center gap-2">
              <Eye className="h-5 w-5 text-amber-600" /> {room.view}
            </span>
          </div>

          <h2 className="mt-8 font-serif text-xl text-slate-900">About This Room</h2>
          <p className="mt-3 leading-relaxed text-slate-600">{room.longDescription}</p>

          <h2 className="mt-10 font-serif text-xl text-slate-900">Room Amenities</h2>
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            {room.amenities.map((amenity) => (
              <div key={amenity} className="flex items-center gap-2.5 text-sm text-slate-600">
          ...
          <div className="mt-10 rounded-2xl bg-slate-50 p-6 sm:p-8">
            ...
            <p className="mt-3 italic leading-relaxed text-slate-600">
              "An absolutely stunning room with impeccable service. Every detail felt considered
              — we will definitely be returning to Lumiere Grande."
            </p>
            <p className="mt-3 text-sm font-semibold text-slate-800">— Verified Guest</p>
          </div>
        </div>
        ...
        <aside className="mt-10 lg:mt-0">
          <div className="sticky top-28 rounded-2xl border border-slate-100 bg-white p-6 shadow-lg shadow-slate-200/50">
            ...
            <ul className="mt-6 space-y-3 border-y border-slate-100 py-5 text-sm text-slate-600">
              ...
              <li className="flex justify-between">
                <span>Size</span>
                <span className="font-medium text-slate-800">{room.size} m²</span>
              </li>
              <li className="flex justify-between">
                <span>View</span>
                <span className="font-medium text-slate-800">{room.view}</span>
              </li>
            </ul>
